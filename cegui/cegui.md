...
有趣的来了。我们希望皮肤能够正确运行，而无需提前知道图像的大小，因此我们必须使用数学计算来确定所需的中间部分宽度。 如果我们确定知道图像的大小，则可以全部进行预先计算，而我们可以简单地使用AbsoluteDim告诉系统所需的宽度。 不幸的是我们并不幸运。 但是，我们很幸运，因为该系统为我们提供了一种在XML内指定应进行哪些计算以得出维的最终值的方法。 提供此功能的是OperatorDim元素。
