...
我们可以采用多种方法将这些图像部分应用于小部件。 尽管最终用途是使端件以其“自然”的水平尺寸绘制，并且将中间部分拉伸以填充两端之间的空间。 听起来很简单，尽管有一个问题。 图像的实际像素大小不是固定的。 TaharezLook图像集使用自动缩放功能，这意味着源图像的大小取决于显示分辨率。 指定图像时，所有这些都需要考虑在内； 这样一来，我们可以确保在所有分辨率下，结果都能达到预期。
