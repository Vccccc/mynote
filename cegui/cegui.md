...
TaharezLook 的图像为每个按钮状态提供了三个部分（“Disabled”除外；为此，我们将仅重新使用“normal_imagery”并使用一些不同的颜色！）。 可用的图像部分为我们提供了一个左端，一个右端和一个中间部分。
...
我们需要放置的最后一个图像是在“normal_imagery”中间部分。 请记住，我们希望该图像占据两个末端之间的空间。 实现此目的的主要部分是正确定义图像的目标区域。

中间部分的图像定义的垂直方面将与两端相同，因此不再赘述。

我们需要做的第一件事就是告诉系统中间部分的左边缘应该出现在哪里。 我们知道中间部分的图像的左边缘需要与左部分的图像的右边缘连接。 为此，我们可以使用 ImageDim 元素获取左端图像的宽度，并将其用作中间部分区域左边缘的坐标：
```c
<Area>
  <Dim type="LeftEdge">
    <ImageDim
...
有趣的来了。我们希望皮肤能够正确运行，而无需提前知道图像的大小，因此我们必须使用数学计算来确定所需的中间部分宽度。 如果我们确定知道图像的大小，则可以全部进行预先计算，而我们可以简单地使用 AbsoluteDim告诉系统所需的宽度。 不幸的是我们并不幸运。 但是，我们很幸运，因为该系统为我们提供了一种在 XML 内指定应进行哪些计算以得出维的最终值的方法。 提供此功能的是 OperatorDim 元素。

在继续之前，我们应该查看我们需要计算的内容。中间部分的宽度基本上是小部件的宽度，减去两个末端部分的总宽度：
```c
...
现在，我们可以在定义各种状态的元素内添加对图像部分的引用。状态图像元素的图像必须在图层中指定。 可以指定每个图层中要使用的多个图像部分，尽管在大多数情况下，您只需要一个图层即可。

在这里，我们为“Normal”状态添加了图像规范：
```c
<StateImagery name="Normal">
  <Layer>
...
但是，Disabled 状态有所不同。我们没有针对该状态的任何特定图像，因此我们将重新使用 normal_imagery，但是将指定一些颜色以使按钮显得更暗。这是通过将 Colors 元素嵌入 Section 元素中来完成的，如此处所示：
```c
<StateImagery name="Disabled">
  <Layer>
    <Section section="normal_imagery">
...
现在，我们有了一个不错的按钮，其中为所有必需状态定义了图像。这里缺少一件事-我们需要在按钮上放置一些标签文本。
...
TextComponent 的定义与 ImageryComponent 的定义极为相似。 我们为文本和所需的格式指定一个区域。我们还可以选择指定 Text 元素，该元素用于显式设置要绘制的字体和(或)文本字符串。 没有这些显式设置，这些项目将从基本窗口小部件本身获取。
...
- Attributes：
	...
	- autoScaled：用于指定如何相对于指定的原始分辨率缩放图像的属性。 原始分辨率值旨在指示将导致从源到目标的一对一（即未缩放）映射的分辨率。 在显示分辨率高于原始分辨率的情况下，图像将按比例放大以占用更多空间，而在显示分辨率低于原始分辨率的情况下，图像将按比例缩小以占用更少的空间。 该属性的可能值如下：
		- vertical - 图像会根据原始分辨率高度和当前分辨率高度自动缩放。 适当计算水平缩放比例以保持纵横比。
		- horizontal - 图像会根据原始分辨率宽度和当前分辨率宽度自动缩放。 适当计算垂直缩放比例以保持纵横比。
		- min - 如上计算垂直和水平缩放因子，并根据两个缩放因子中较小的一个对图像进行自动缩放。 纵横比保持不变。
		- max - 如上计算垂直和水平缩放因子，并根据两个缩放因子中较大的一个对图像进行自动缩放。 纵横比保持不变。
		- true - 垂直和水平缩放因子的计算方法如上所述，图像高度和宽度分别缩放。 无法保持长宽比。
		- false - 不会执行图像的自动缩放。 可选属性，默认为false。
	- nativeHorzRes：启用“autoScaled”后，指定 horizontal 屏幕分辨率，该分辨率将导致像素从源图像到目标表面的不缩放，一对一映射。 可选属性，默认为640。
	- nativeVertRes：启用“autoScaled”后，指定垂直屏幕分辨率，该分辨率将导致像素从源图像到目标表面的未缩放比例的一对一映射。 可选属性，默认为480。
