...
CEGUI 中可用于窗口的许多设置和属性都沿窗口层次结构传递。例如，如果将特定窗口的 Alpha 透明度设置为 0.5，则默认情况下，所有窗口和附加到该窗口的小部件也将受到较高级别应用的更改的影响；尽管还请注意子窗口上的实际设置保持不变-最终值和/或设置通常是从层次结构中所有窗口到当前窗口的设置值的某种组合。这也适用于诸如窗户破坏之类的事情。默认情况下，窗口在销毁时会销毁附加的子窗口和小部件。这种安排的主要优点是，您可以通过更改根窗口设置（例如Alpha，可见性，启用/禁用状态）来轻松影响整个GUI，并可以通过简单地销毁整个GUI布局来轻松“清理”整个GUI布局。根窗口。在需要更多细粒度控制或要使用某些高级管理技术的情况下，可以在每个窗口的基础上覆盖默认的“继承”行为。