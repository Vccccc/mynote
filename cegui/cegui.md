...
#### LookNFeel
LookNFeel 文件构成 CEGUI 外观系统的主要部分。 每个组件都将包含一个或多个 WidgetLook 定义，这些定义向系统描述了各种图像和子窗口小部件组件，它们的位置，其他属性值等等。 这些WidgetLook 规范随后绑定到系统中的具体元素，以形成最终表示为 CEGUI中 的窗口和小部件的东西。
