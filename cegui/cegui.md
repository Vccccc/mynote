...
对于任何 CEGUI 应用程序（以及您的布局文件），建议的 root 窗口都是 DefaultWindow，它覆盖了整个应用程序窗口。 只需将窗口的大小设置为 1.0 的相对宽度和高度，并将位置设置为原点（0,0），即可使其覆盖整个窗口。 这是新创建的DefaultWindows的默认设置，因此不必专门设置此类窗口。 创建窗口后，可以随时将其设置为根窗口。 始终只能有一个根窗口。 所有其他窗口都应附加到该窗口。