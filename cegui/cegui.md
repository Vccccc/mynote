...
GUIContext :: setRootWindow函数用于指定一个窗口，以用作给定GUIContext的GUI根。 这将替换任何当前的根窗口，尽管要注意，先前的窗口层次实际上并没有被破坏-它只是从GUIContext中分离出来的-您可以通过使用GUIContext :: setRootWindow函数在GUI“页面”之间进行切换来轻松地在它们之间切换。