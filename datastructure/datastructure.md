...
1. 邻接矩阵。可以使用一个 V*V 的布尔矩阵。当顶点 v 和 顶点 w 之间有相连接的边时，定义 v 行 w 列的元素值为 true，否则为 false。这种表示方法不符合第一个条件--含有上百万个顶点的图是很常见的， V^2 个布尔值所需的空间是不能够满足的。
2. 边的数组。可以使用一个 Edge 类，它含有两个 int 变量。这种方法很简洁但不满足第二个条件--要实现一个找出与某个节点相邻的所有顶点的方法需要检查图中的所有边。
3. 邻接表数组。可以使用一个一顶点为索引的列表数组，其中的每个元素都是和该顶点相邻的顶点列表。这种数据节构能够同时满足典型应用所需的以上两个条件。
