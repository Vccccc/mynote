...
第一行Num表示符号表数组的下标，从0开始，共15个字符;第二列Value就是符号值，即st_value;第三列Size为符号大小，即st_size;第四列和第五列分别为符号类型和绑定信息，即对应st_info的底4位和高28位;第六列Vis目前在C/C++语言中未使用;第七列Ndx，即st_shndx，表示该符号所属的段;最后一列是符号名称。第一个符号，即下标为0