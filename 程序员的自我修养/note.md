...
这种符号的定义被称为强符号（Strong symbol）。有些符号的定义可以被称为弱符号（Weak symbol）。对于C/C\++语言来说，编译器默认函数和初始化了的全局变量为强符号，未初始化的全局变量为弱符号。可以通过GCC的“__attribute\__((weak))”来定义任何一个强符号为弱符号。注意，强符号和弱符号都是针对定义