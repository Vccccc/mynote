...
- 1, 小端格式
- 2, 大端格式
最开始的4个字节是所有ELF文件都必须相同的标识码，分别为0x7F、0x45、0x4c、0x46，第1个字节对应ASCII字符里的DEL控制符，后面3个字节刚好是ELF这3个字母的ASCII码。这4个字节又被称为ELF文件的魔数，几乎所有的可执行文件格式的最开始的几个字节都是魔数。比如a.out格式最开始的两个字节为0x01、0x07;PE/COFF文件最开始两个字节为0x4d、0x5a。这种魔数用于确认文件的类型，操作
