...
#### C++符号修饰
因为C++拥有类、继承、虚机制、重载、名称空间等特性，它们使得符号管理更为复杂。最简单的例子，两个相同名字的函数func(int)和func(double)，尽管函数名相同，但是参数列表不同。为了编译器和链接器在链接过程中区分这些符号，人们发明了**符号修饰(Name Decoration)**或**符号改编(Name Mang**
