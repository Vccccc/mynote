...
调度算法试图最大化 throughput 可能并不需要最小化 turnaround time。比如，给定混合短时间和长时间的作业，调度器只需总是运行短时间的作业，从不运行长时间的作业可能会达到很好的 throughput（每小时完成许多短作业），但是对于长作业而言，turnaround time 会很烂。
