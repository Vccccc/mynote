...
Batch system 仍然广泛应用在商业世界，用于工资账单、库存、应收账款、应付账款、利息计算（银行）、索赔处理（保险公司）以及其他周期性服务。在 Batch system 中，没有用户会不耐烦地要求终端快速响应。结论是，无论是抢占式算法还是非抢占式算法，只要是有一个足够长的周期给每个进程都是可以接受的，