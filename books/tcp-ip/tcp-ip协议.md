...
拥塞避免算法要求每次收到一个确认时将 cwnd 增加 1 个 cwnd。与慢启动的指数增加比起来，这是一种加性增长（additive increase）。我们希望在一个往返时间内最多为 cwnd 增加 1 个报文段（不过在这个 RTT 中收到了多少个 ACK），然而慢启动将根据这个往返时间中所收到的确认的
